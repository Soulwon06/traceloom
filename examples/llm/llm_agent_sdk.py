"""Claude Agent SDK example — and a demonstration of traceloom's capture boundary.
...
api.anthropic.com. traceloom monkey-patches httpx/requests *in-process*, so it does
not see traffic that originates in a child process.

Expected result: this script runs and prints Claude's answer, but NO new
`POST /v1/messages` event shows up in the traceloom dashboard — the LLM call happened
...
async def main() -> None:
    ...
    print(
        ...
        "the Agent SDK's API call ran in the `claude` subprocess, outside traceloom's reach."
    )
