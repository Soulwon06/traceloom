"""Demo: Django server with TraceLoom capturing incoming requests, outgoing HTTP, logs, and exceptions.

Start the TraceLoom server first (uv run traceloom-server), then run this example:

    uv run examples/python/django_server.py

Open http://localhost:8000/ to try the endpoints, then check
http://localhost:5110 to see everything TraceLoom captured.
"""
...
settings.configure(
    ...
    ALLOWED_HOSTS=["*"],
    SECRET_KEY="traceloom-demo",
    MIDDLEWARE=[
        "traceloom.integrations.django.TraceLoomMiddleware",
    ],
    TRACELOOM_IGNORE_PATHS=["/favicon.ico"],
)
django.setup()
...
def index(request):
    ...
    html = (
        "<h1>TraceLoom Django Demo</h1>"
        f"<ul>{links}</ul>"
        '<p>Open <a href="http://localhost:5110">localhost:5110</a>'
        " to see what TraceLoom captured.</p>"
    )
...
def fetch_external(request):
    """Makes an outgoing HTTP request so TraceLoom captures both sides."""
    url = request.GET.get("url", "https://httpbin.org/get")
    demo_logger.info("fetching external URL", extra={"url": url})
    resp = httpx.get(url, params={"source": "traceloom-demo"})
...
def crash(request):
    """Raises an unhandled exception — TraceLoom captures it with exc_type/exc_value."""
...
if __name__ == "__main__":
    ...
    print("Open http://localhost:5110 to see what TraceLoom captured.\n")
