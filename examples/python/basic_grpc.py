"""Example: Capture gRPC calls with TraceLoom.
...
def main():
    ...
    with grpc.insecure_channel(GRPCBIN_HOST) as channel:
        ...
        resp = stub.DummyUnary(
            grpcbin_pb2.DummyMessage(f_string="hello from traceloom", f_int32=42)
        )
    ...
    try:
        with grpc.insecure_channel(GRPCBIN_HOST) as channel:
            stub = grpcbin_pb2_grpc.GRPCBinStub(channel)
            stub.SpecificError(
                grpcbin_pb2.SpecificErrorRequest(code=5, reason="testing traceloom")
            )
    except grpc.RpcError as e:
        print(f"Got expected error: {e.code().name} — {e.details()}")

    # Flush captures before exiting
    traceloom.flush(timeout=3)
