"""Demo: send a variety of requests to httpbin.org and inspect them in TraceLoom."""
...
resp = requests.get(
    "https://httpbin.org/get", params={"lang": "python", "tool": "traceloom"}
)
print(f"GET  /get          → {resp.status_code}")

# POST with JSON body
resp = requests.post(
    "https://httpbin.org/post", json={"message": "hello from traceloom", "count": 42}
)
...
resp = requests.get(
    ...
    headers={"X-Custom-Header": "traceloom-demo", "Authorization": "Bearer sk-secret-key"},
)
...
resp = requests.get(
    ...
    params={"q": "traceloom", "api_key": "sk-1234567890", "token": "xoxb-secret-token"},
)
