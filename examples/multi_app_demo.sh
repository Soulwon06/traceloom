...
#
# Prerequisites:
#   traceloom-server running on http://localhost:5110
#   pip install traceloom
...
uv run traceloom run --app requests-app --session "$SESSION" \
  python examples/python/basic_requests.py

echo "Running httpx demo as 'httpx-app'..."
uv run traceloom run --app httpx-app --session "$SESSION" \
