...
TraceLoom supports Gmail/Linear-style keyboard shortcuts for fast navigation.
...
hotkeys/
  ...
  useListNavigation.ts   -- hierarchy-aware arrow-key and j/k navigation
...
List navigation uses the same hierarchy projection and shared collapse state as `RequestList`.
Up and Down visit rendered events in tree order and skip descendants of collapsed nodes.
Right expands the selected operation, then enters its first visible child. Left collapses an
expanded operation, then moves to its runtime parent. Virtual groups affect which events are
visible but aren't selectable keyboard destinations.
