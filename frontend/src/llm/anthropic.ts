...
/**
 * Some evaluation frameworks interpolate variables as JSON string literals:
 * `inputs: "line one\nline two"`. Decode those values for the LLM view while
 * leaving the Tree and Raw tabs untouched.
 */
function decodeSerializedPromptFields(text: string): string {
  let decodedAny = false;
  const decodedLines = text.split("\n").map((line) => {
    const match = line.match(/^(\s*[A-Za-z_][\w.-]*\s*):\s*("(?:[^"\\]|\\.)*")\s*$/);
    if (!match || (!match[2]!.includes("\\n") && !match[2]!.includes('\\"'))) return line;

    const decoded = tryParseJson(match[2]!);
    if (typeof decoded !== "string") return line;
    decodedAny = true;
    return `${match[1]}:\n${decoded}`;
  });
  return decodedAny ? decodedLines.join("\n") : text;
}
...
function contentToBlocks(content: unknown): LlmBlock[] {
  if (typeof content === "string") {
    return content ? [{ kind: "text", text: decodeSerializedPromptFields(content) }] : [];
  }
