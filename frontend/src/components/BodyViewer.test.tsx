...
describe("BodyViewer", () => {
  ...
  it("renders JSON with Tree/Raw tabs", () => {
    ...
    expect(view.getByRole("tab", { name: "结构视图 Tree" })).toBeInTheDocument();
    expect(view.getByRole("tab", { name: "原始数据 Raw" })).toBeInTheDocument();
  ...
  it("switches to Raw tab for JSON", async () => {
    const { container } = render(<BodyViewer data='{"key":"value"}' />);
    const view = within(container);
    await userEvent.click(view.getByRole("tab", { name: "原始数据 Raw" }));
  ...
  it("renders an LLM tab for a Gemini response body", () => {
    const body = JSON.stringify({
      candidates: [
        {
          content: {
            role: "model",
            parts: [{ text: "Order 1042 has shipped and should arrive on July 30." }],
          },
          finishReason: "STOP",
        },
      ],
    ...
    expect(container.textContent).toContain("Order 1042 has shipped and should arrive on July 30.");
    expect(container.textContent).toContain("2 thinking");
  });
