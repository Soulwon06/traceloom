  ...
  return (
    ...
    >
      ...
      >
        ...
        <Tooltip title="在 VS Code 中打开 Open in VS Code">
          <Box
            component="a"
    ...
    <Box sx={{ p: 2, overflowY: "auto" }}>
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 0.5 }}>
        <Stack direction="row" alignItems="center" spacing={0.75}>
          <BilingualLabel title="调用栈" english="Traceback" />
          <TermExplanation
            title="调用栈"
            english="Traceback"
            what="表示异常发生时，程序经过的调用路径和上下文。"
            why="它可以帮助定位异常是从哪里传播出来的。"
            how="先查看最相关的栈帧，再结合原始 traceback 和代码上下文继续检查。"
          />
        </Stack>
      ...
      {d.frames && d.frames.length > 0 && (
        <>
          <Stack direction="row" alignItems="center" spacing={0.75} sx={{ mb: 0.5 }}>
            <BilingualLabel title="栈帧" english={`Frames (${d.frames.length})`} />
            <TermExplanation
              title="栈帧"
              english="Frames"
              what="调用栈中的单个调用位置，通常包含文件、行号和函数名。"
              why="它可以帮助把异常调用路径拆成可检查的具体位置。"
              how="可以展开有代码上下文的栈帧，并保留文件名、行号和函数名等原始信息。"
            />
          </Stack>
