...
export default function HttpIncomingDetail({
  ...
  const statusDescription = statusDescriptions[d.status_code];
      ...
      <Box sx={{ mb: 2 }}>
        <Box sx={{ minWidth: 0, mb: 1.5 }}>
          ...
          <Stack direction="row" alignItems="baseline" spacing={1} sx={{ mt: 0.75, minWidth: 0 }}>
            ...
            <Typography sx={{ fontFamily: mono, fontSize: 14, fontWeight: 500, wordBreak: "break-all", minWidth: 0 }}>
              {d.path}
            </Typography>
          </Stack>
        </Box>
        ...
        <Stack direction="row" useFlexGap spacing={2} sx={{ flexWrap: "wrap", alignItems: "flex-start" }}>
          ...
          <Box sx={{ minWidth: 120, flex: "1 1 120px" }}>
            <BilingualLabel title="框架" english="Framework" />
            <Chip label={d.framework} size="small" variant="outlined" sx={{ mt: 0.5, fontSize: 12 }} />
          </Box>
          {d.route && (
            <Box sx={{ minWidth: 160, flex: "1 1 160px" }}>
              <BilingualLabel title="路由" english="Route" />
              <Typography sx={{ fontFamily: mono, fontSize: 12, mt: 0.5, wordBreak: "break-word" }}>
                {d.route}
              </Typography>
            </Box>
          )}
          {d.client_ip && (
            <Box sx={{ minWidth: 140, flex: "1 1 140px" }}>
              <BilingualLabel title="客户端 IP" english="Client IP" />
              <Typography sx={{ fontFamily: mono, fontSize: 12, mt: 0.5, wordBreak: "break-word" }}>
                {d.client_ip}
              </Typography>
            </Box>
