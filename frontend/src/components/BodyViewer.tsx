    ...
    return (
      <Box>
        <Stack direction="row" alignItems="center" spacing={0.5} sx={{ minWidth: 0 }}>
          <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ ...tabsSx, flex: 1, minWidth: 0 }}>
            <Tab label="LLM" />
            <Tab label="结构视图 Tree" />
            <Tab label="原始数据 Raw" />
          </Tabs>
          <TermExplanation
            title="原始数据"
            english="Raw JSON"
            what="表示未经结构化展示的原始 JSON 内容。"
            why="它可以帮助核对模型或接口实际收到、返回的字段和值。"
            how="当结构化视图与预期不一致时，可以对照 Raw JSON 检查原始字段。"
          />
        </Stack>
    ...
    return (
      <Box>
        <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={tabsSx}>
          <Tab label="LLM" />
          <Tab label={`事件 Events (${body.events.length})`} />
          <Tab label="原始数据 Raw" />
        </Tabs>
        {tab === 0 && <LlmView view={body.view} />}
