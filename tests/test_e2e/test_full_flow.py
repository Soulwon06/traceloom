...
Spins up a real TraceLoom server, patches HTTP libraries via the SDK,
...
from traceloom.patches.patch_aiohttp import patch_aiohttp
from traceloom.patches.patch_httpx import patch_httpx
from traceloom.patches.patch_requests import patch_requests
from traceloom.transport import start_worker
from traceloom_server.app import create_app
...
@pytest.fixture()
def traceloom_server(tmp_path):
    """A real TraceLoom server running on a random port with a fresh DB."""
...
@pytest.fixture()
def patched_requests(traceloom_server):
    """Patch the requests library via traceloom SDK, return the reloaded module."""
    importlib.reload(requests_lib)

    start_worker(traceloom_server)
    config = TraceLoomConfig(server_url=traceloom_server, redact_headers=["authorization"])
    patch_requests(config)
    return requests_lib


@pytest.fixture()
def patched_httpx(traceloom_server):
    """Patch httpx (sync + async) via traceloom SDK, return the reloaded module."""
...
@pytest.fixture()
def patched_aiohttp(traceloom_server):
    """Patch aiohttp via traceloom SDK, return the reloaded module."""
...
def _list_events(traceloom_url, query=""):
    """Return the rows from the `GET /api/events` envelope."""
    return _fetch_json(f"{traceloom_url}/api/events{query}")["events"]
...
def _list_http_events(traceloom_url):
    """Return all captured HTTP events with their data flattened to the top level."""
    summaries = _list_events(traceloom_url, "?event_type=http")
    return [
        _flatten_http_event(_fetch_json(f"{traceloom_url}/api/events/{s['id']}"))
        for s in summaries
    ]


def _wait_for_capture(traceloom_url, expected_count=1, timeout=5):
    """Poll the API until the expected number of captured HTTP events appear."""
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        events = _list_http_events(traceloom_url)
        if len(events) >= expected_count:
            return events
        time.sleep(0.1)
    return _list_http_events(traceloom_url)


def _fetch_event_detail(traceloom_url, event_id):
    """Fetch a single event and flatten its HTTP `data` to the top level."""
    return _flatten_http_event(_fetch_json(f"{traceloom_url}/api/events/{event_id}"))


# -- Tests --------------------------------------------------------------------


def test_request_during_pytest_inherits_test_hierarchy(
    traceloom_server,
    mock_target,
    tmp_path,
):
    ...
    test_file.write_text(
        ...
        "import traceloom\n\n"
        "traceloom.init()\n\n"
        "@pytest.mark.parametrize('currency', ['usd'], ids=['usd'])\n"
        "def test_total(currency):\n"
        "    assert currency == 'usd'\n"
        "    assert requests.get(os.environ['TARGET_URL']).status_code == 200\n"
    )
    env = {
        **os.environ,
        "TRACELOOM_URL": traceloom_server,
        "TARGET_URL": mock_target.replace("127.0.0.1", "localhost"),
    ...
    test_summaries = _list_events(traceloom_server, "?event_type=test")
    http_summaries = _list_events(traceloom_server, "?event_type=http")
    test_detail = _fetch_json(f"{traceloom_server}/api/events/{test_summaries[0]['id']}")
    http_detail = _fetch_json(f"{traceloom_server}/api/events/{http_summaries[0]['id']}")

    # Assert
    assert result.returncode == 0, result.stdout + result.stderr
    assert len(test_summaries) == 1
    assert len(http_summaries) == 1
    assert test_detail["data"]["parameter_id"] == "usd"
    test_runtime = test_detail["hierarchy"]["runtime"]
    assert test_runtime["role"] == "operation"
    assert test_runtime["parent_span_id"] is None
    membership = test_detail["hierarchy"]["group_memberships"][0]
    assert membership["kind"] == "pytest"
    assert membership["test_file"] == {
        "id": "test_checkout.py",
        "label": "test_checkout.py",
    }
    assert membership["test_case"] == {
        "id": "test_checkout.py::test_total",
        "label": "test_total",
    }
    http_runtime = http_detail["hierarchy"]["runtime"]
    assert http_runtime["trace_id"] == test_runtime["trace_id"]
    assert http_runtime["parent_span_id"] == test_runtime["span_id"]
    assert [item["kind"] for item in http_detail["hierarchy"]["group_memberships"]] == [
        "pytest",
        "http_request",
    ]
    assert http_detail["hierarchy"]["group_memberships"][1]["hostname"] == {
        "id": "localhost",
        "label": "localhost",
    }


def test_short_lived_wrapped_process_flushes_http_capture(
    traceloom_server, mock_target, tmp_path
):
    """A wrapped child must deliver an HTTP event before its process exits."""
    test_file = tmp_path / "short_lived_request.py"
    trace_server_url = traceloom_server.replace("127.0.0.1", "localhost")
    test_file.write_text(
        "import os\n"
        "import requests\n\n"
        "response = requests.get(os.environ['TARGET_URL'], timeout=5)\n"
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    data = _wait_for_capture(traceloom_server)
    assert any(event["method"] == "GET" for event in data)


def test_requests_get_captured(traceloom_server, mock_target, patched_requests):
...
def test_httpx_sync_captured(traceloom_server, mock_target, patched_httpx):
...
def test_httpx_async_captured(traceloom_server, mock_target, patched_httpx):
...
def test_redacted_headers_in_api(traceloom_server, mock_target, patched_requests):
    ...
    data = _wait_for_capture(traceloom_server)
    detail = _fetch_event_detail(traceloom_server, data[0]["id"])
...
def test_aiohttp_get_captured(traceloom_server, mock_target, patched_aiohttp):
    async def make_request():
...
def test_aiohttp_post_captured(traceloom_server, mock_target, patched_aiohttp):
    async def make_request():
        async with patched_aiohttp.ClientSession() as session:
...
def test_aiohttp_post_str_data(traceloom_server, mock_target, patched_aiohttp):
    async def make_request():
        async with patched_aiohttp.ClientSession() as session:
            async with session.post(
...
def test_aiohttp_post_bytes_data(traceloom_server, mock_target, patched_aiohttp):
    async def make_request():
        async with patched_aiohttp.ClientSession() as session:
            async with session.post(
...
def test_aiohttp_ignored_host_not_captured(traceloom_server, mock_target):
    ...
    time.sleep(1)
    data = _list_http_events(traceloom_server)
...
def test_aiohttp_response_readable_after_capture(
    traceloom_server, mock_target, patched_aiohttp
):
...
def test_aiohttp_streaming_not_consumed(traceloom_server, mock_target, patched_aiohttp):
...
def test_aiohttp_custom_json_serializer(traceloom_server, mock_target, patched_aiohttp):
...
def test_aiohttp_base_url_ignore_hosts(traceloom_server, mock_target):
...
def test_aiohttp_session_headers_captured(traceloom_server, mock_target, patched_aiohttp):
    ...
    detail = _fetch_event_detail(traceloom_server, matching[0]["id"])
    headers_lower = {k.lower(): v for k, v in detail["request_headers"].items()}
    assert headers_lower["x-session-default"] == "from-session"
    assert headers_lower["x-per-request"] == "per-req"


def test_aiohttp_raise_for_status_captured(traceloom_server, mock_target, patched_aiohttp):
    ...
    detail = _fetch_event_detail(traceloom_server, matching[0]["id"])
    assert detail["error"]["type"] == "ClientResponseError"
    assert detail["response_headers"]


def test_aiohttp_post_dict_data(traceloom_server, mock_target, patched_aiohttp):
...
def test_aiohttp_redirect_hops_captured(traceloom_server, mock_target, patched_aiohttp):
    ...
    data = _wait_for_capture(traceloom_server, expected_count=2)
...
def test_aiohttp_redirect_method_change(traceloom_server, mock_target, patched_aiohttp):
    ...
    detail = _fetch_event_detail(traceloom_server, final[0]["id"])
    assert detail["request_body"] is None


def test_aiohttp_close_without_read(traceloom_server, mock_target, patched_aiohttp):
