# Debug OpenAI with TraceLoom

The OpenAI Python SDK wraps a complex HTTP API behind convenient methods like `client.chat.completions.create()`. When something goes wrong (unexpected responses, rate limits, token overages), you need to see the raw API calls. TraceLoom captures the HTTP requests that the OpenAI SDK makes through httpx, showing you exactly what's sent and received.
...
```bash
traceloom run my_openai_app.py
```

The OpenAI SDK uses `httpx` under the hood. TraceLoom's httpx patch captures all API calls automatically. No code changes needed.
...
Open the TraceLoom dashboard and click the request to `api.openai.com/v1/chat/completions`:

![TraceLoom dashboard showing captured OpenAI API calls](assets/debug-openai-dashboard.png)
...
For OpenAI requests and responses, TraceLoom detects the chat shape and defaults to an **LLM** tab that renders the call as a conversation instead of raw JSON:

![TraceLoom LLM conversation view for an OpenAI chat completion](assets/debug-openai-llm.png)
...
![Claude Code session using traceloom to diagnose high token usage](assets/debug-openai-claude.png)
...
- **Streaming responses**: When using `stream=True`, TraceLoom captures the initial request and the full response. You can see the complete streamed output in the response body, even though your code received it token by token.
...
- **Function calling / tools**: If you're using function calling, the tool definitions are part of the request body and the tool call responses are in the response body. TraceLoom shows both, so you can debug tool-use flows.
...
- **Retries**: The OpenAI SDK retries on certain errors automatically. Each attempt appears as a separate request in TraceLoom, so you can see what was retried and why.
