# Debug Stripe with TraceLoom

Stripe's Python SDK makes HTTP calls to `api.stripe.com` using the `requests` library. When charges fail, webhooks misbehave, or idempotency keys cause confusion, you need to see the raw API traffic. TraceLoom captures every Stripe API call automatically.
...
```bash
traceloom run my_stripe_app.py
```

Stripe's Python SDK uses `requests` under the hood (via `urllib3`). TraceLoom captures all API calls automatically. No code changes needed.
...
Open the TraceLoom dashboard and filter to `api.stripe.com`:

![TraceLoom dashboard showing captured Stripe API calls](assets/debug-stripe-dashboard.png)
...
![Claude Code session using traceloom to diagnose a silent payment failure](assets/debug-stripe-claude.png)
...
- **Webhook debugging**: If you're using TraceLoom's FastAPI middleware to capture incoming requests, you'll see Stripe webhook `POST` deliveries in the timeline alongside your outgoing API calls. This gives you both sides of the conversation.
...
- **Stripe-Request-Id**: Every Stripe response includes a `Request-Id` header. When contacting Stripe support, you can grab this directly from the TraceLoom dashboard.
