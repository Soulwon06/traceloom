# Debug Google Cloud with TraceLoom

Google Cloud client libraries for Python use gRPC under the hood for services like BigQuery, Firestore, Pub/Sub, and Cloud Storage. TraceLoom captures these gRPC calls automatically, showing you the protobuf payloads as readable JSON.
...
TraceLoom patches `grpc.secure_channel`, which is what Google Cloud client libraries use to communicate with Google's APIs. No code changes needed.
...
Open the TraceLoom dashboard. You'll see multiple gRPC calls that the BigQuery client makes:

![TraceLoom dashboard showing captured Google Cloud gRPC calls](assets/debug-google-cloud-dashboard.png)
...
![Claude Code session using traceloom to diagnose slow BigQuery queries](assets/debug-google-cloud-claude.png)
...
- **REST fallback**: Some Google Cloud libraries support a REST transport as an alternative to gRPC. If you're using REST transport, TraceLoom captures those via the `requests` or `httpx` patches instead.
