...
def patch_grpc(config: TraceLoomConfig) -> None:
...
def _make_interceptor_class(base_class):
    ...
    class _TraceLoomInterceptor(base_class):
        """gRPC unary-unary client interceptor that captures calls."""

        def __init__(self, config: TraceLoomConfig, target: str):
            self._config = config
            self._target = target
...
def _intercept_unary_unary(config, target, continuation, client_call_details, request):
    ...
    try:
        ...
        duration = time.monotonic() - start
        grpc_code, grpc_name = _grpc_error_status(err)
...
def _send_error_capture(
    *,
    ...
    send_http(payload, context=context)
    logger.debug("captured POST %s via grpc (%s)", url, type(error).__name__)


def _grpc_error_status(error: Exception) -> tuple[int, str]:
    """Return the numeric and named gRPC status carried by an RPC error."""
    grpc_code = 2
    grpc_name = "UNKNOWN"
    try:
        ...
        if isinstance(value, tuple) and value:
            grpc_code = int(value[0])
        grpc_name = getattr(code, "name", grpc_name)
    except Exception:
        pass
    return grpc_code, grpc_name
