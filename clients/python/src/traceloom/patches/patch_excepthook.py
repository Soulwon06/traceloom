...
def patch_excepthook(config: TraceLoomConfig) -> None:
    ...
    def traceloom_excepthook(exc_type, exc_value, exc_tb):
        capture_exception(exc_type, exc_value, exc_tb)
        transport.flush(timeout=2.0)
        original_excepthook(exc_type, exc_value, exc_tb)

    sys.excepthook = traceloom_excepthook
    ...
    def traceloom_threading_excepthook(args):
        capture_exception(args.exc_type, args.exc_value, args.exc_traceback)
        original_threading_excepthook(args)

    threading.excepthook = traceloom_threading_excepthook
