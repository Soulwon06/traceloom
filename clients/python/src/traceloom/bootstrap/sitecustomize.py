"""Auto-init TraceLoom when this directory is on PYTHONPATH.

Python's ``site`` module imports a top-level ``sitecustomize`` module at
interpreter startup, before any user code runs. ``traceloom run`` prepends the
...
The bootstrap is defensive: if anything fails (traceloom not importable, no
...
call ``traceloom.init()`` directly from it instead of using ``traceloom run``.
"""
...
def _activate() -> None:
    if not os.environ.get("TRACELOOM_URL"):
        return
    try:
        import traceloom  # noqa: PLC0415
