...
from traceloom.patches import apply_all as _apply_all
from traceloom.transport import flush, shutdown
from traceloom.transport import start_worker as _start_worker

logger = logging.getLogger("traceloom")
logger.addHandler(logging.NullHandler())

__all__ = ["init", "flush", "is_test_capture_enabled", "shutdown"]
__version__ = "0.18.0"

DEFAULT_REDACT_HEADERS = ["authorization", "x-api-key", "x-goog-api-key"]

_config: TraceLoomConfig | None = None
_patched: bool = False
_atexit_registered: bool = False


def _load_cli_provenance() -> dict[str, str | None]:
    """Read and consume ``_TRACELOOM_CLI_PROVENANCE`` set by ``traceloom run``.

    The env var is removed after reading so it does not leak to
    grandchild processes that call ``init()`` independently.
    """
    raw = os.environ.pop("_TRACELOOM_CLI_PROVENANCE", None)
    if not raw:
        return {}
    try:
...
def _env_provenance(env_var_name: str, cli_prov: dict[str, str | None]) -> str:
    ...
    env var name (e.g. ``"TRACELOOM_URL"``).
    """
...
def init(
    ...
    """Initialize TraceLoom. Patches HTTP libraries, logging, and exception hooks.

    TraceLoom only activates when a server URL is provided — either via the
    ``server_url`` parameter or the ``TRACELOOM_URL`` environment variable.
    Without a URL, ``init()`` is a no-op: no monkey-patching, no background
    threads, no side effects.  This makes it safe to leave ``traceloom.init()``
    in production code and control activation purely via the environment.

    Each parameter falls back to a ``TRACELOOM_*`` environment variable when
    ...
    When ``debug`` is enabled, TraceLoom logs its resolved configuration,
    library patching, capture decisions, and transport activity to stderr
    via the ``"traceloom"`` Python logger.  You can also configure this logger
    manually (e.g. ``logging.getLogger("traceloom").setLevel(logging.DEBUG)``)
    without setting the ``debug`` flag.

    ``app`` tags every captured event with an application name, useful when
    multiple services share a single TraceLoom server. ``session`` tags events
    ...
    The same applies to the ``TRACELOOM_LOG_LEVEL`` environment variable.

    .. note::

       ``log_level`` is a capture filter, not a logger override. It controls
       which records TraceLoom keeps after they pass through Python's normal
    ...
    monkey-patches; subsequent calls update the live ``TraceLoomConfig`` in
    place so new args (filtering, redaction) take effect immediately,
    without re-wrapping already patched methods. This makes ``traceloom run``
    safe to use on programs that already call ``traceloom.init()`` themselves.
    """
    ...
    if debug is not None:
        ...
        if env_val is not None:
            debug = env_val
            provenance["debug"] = _env_provenance("TRACELOOM_DEBUG", cli_prov)
        else:
            debug = False
    ...
    if server_url is not None:
        ...
        if server_url:
            provenance["server_url"] = _env_provenance("TRACELOOM_URL", cli_prov)
        else:
            provenance["server_url"] = "default"
    if not server_url:
        logger.warning(
            "traceloom.init() called without a server URL. "
            "Set TRACELOOM_URL or pass server_url= to activate. Doing nothing."
        )
        return
    ...
    if capture_all is not None:
        ...
        if env_val is not None:
            capture_all = env_val
            provenance["capture_all"] = _env_provenance("TRACELOOM_CAPTURE_ALL", cli_prov)
        else:
            capture_all = True
    ...
    if capture_hosts is not None:
        ...
        provenance["capture_hosts"] = (
            _env_provenance("TRACELOOM_CAPTURE_HOSTS", cli_prov) if env_val else "default"
        )
    ...
    if ignore_hosts is not None:
        ...
        provenance["ignore_hosts"] = (
            _env_provenance("TRACELOOM_IGNORE_HOSTS", cli_prov) if env_val else "default"
        )
    ...
    if redact_headers is not None:
        ...
        provenance["redact_headers"] = (
            _env_provenance("TRACELOOM_REDACT_HEADERS", cli_prov)
    ...
    if redact_query_params is not None:
        ...
        provenance["redact_query_params"] = (
            _env_provenance("TRACELOOM_REDACT_QUERY_PARAMS", cli_prov)
            if env_val
            else "default"
        )
    ...
    if capture_exceptions is not None:
        ...
        if env_val is not None:
            ...
            provenance["capture_exceptions"] = _env_provenance(
                "TRACELOOM_CAPTURE_EXCEPTIONS", cli_prov
            )
        else:
            capture_exceptions = True
            provenance["capture_exceptions"] = "default"

    if capture_tests is not None:
        provenance["capture_tests"] = "param"
    else:
        env_val = env_bool("CAPTURE_TESTS")
        if env_val is not None:
            capture_tests = env_val
            provenance["capture_tests"] = _env_provenance(
                "TRACELOOM_CAPTURE_TESTS", cli_prov
            )
        else:
            capture_tests = True
            provenance["capture_tests"] = "default"
    ...
    if capture_logs is not None:
        ...
        if env_val is not None:
            ...
            provenance["capture_logs"] = _env_provenance(
                "TRACELOOM_CAPTURE_LOGS", cli_prov
            )
        else:
            capture_logs = False
    ...
    if ignore_loggers is not None:
        ...
        provenance["ignore_loggers"] = (
            _env_provenance("TRACELOOM_IGNORE_LOGGERS", cli_prov) if env_val else "default"
        )
    ...
    if log_level is not None:
        ...
        if env_val is not None:
            log_level = env_val
            provenance["log_level"] = _env_provenance("TRACELOOM_LOG_LEVEL", cli_prov)
        else:
            log_level = logging.WARNING
    ...
    if app is not None:
        ...
        app = env_val or ""
        provenance["app"] = (
            _env_provenance("TRACELOOM_APP", cli_prov) if env_val else "default"
        )
    ...
    if session is not None:
        ...
        session = env_val or ""
        provenance["session"] = (
            _env_provenance("TRACELOOM_SESSION", cli_prov) if env_val else "default"
        )
    ...
    if _config is None:
        ...
        _config.capture_tests = capture_tests
    ...
    # Always ignore the traceloom server itself
...
def is_test_capture_enabled() -> bool:
    """Return whether TraceLoom is active and configured to capture pytest tests."""
    return _config is not None and _config.capture_tests
