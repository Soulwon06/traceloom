...
from traceloom.patches import patch_logging as patch_logging_mod


def _make_config(**kwargs):
    defaults = {"capture_logs": True, "log_level": logging.DEBUG}
    defaults.update(kwargs)
    return TraceLoomConfig(server_url="http://localhost:5110", **defaults)
...
def test_ignores_traceloom_loggers(mock_transport):
    # Arrange
    config = _make_config()
    patch_logging_mod.patch_logging(config)
    traceloom_logger = logging.getLogger("traceloom.internal")
    traceloom_logger.setLevel(logging.DEBUG)

    # Act
    traceloom_logger.warning("internal noise")
