"""Tests for traceloom.patches.patch_grpc — gRPC interception."""
...
from traceloom.hierarchy import start_root
from traceloom.patches.patch_grpc import (
    GRPC_STATUS_TO_HTTP,
    _extract_host,
...
def test_make_interceptor_class_delegates_to_intercept(config):
    ...
    with patch("traceloom.patches.patch_grpc._intercept_unary_unary") as mock_fn:
...
@patch("traceloom.patches.patch_grpc.send_http")
@patch("traceloom.patches.patch_grpc.serialize_request_response")
def test_interceptor_captures_successful_call(mock_serialize, mock_send, config):
    ...
    mock_send.assert_called_once_with({"id": "test"}, context=ANY)
...
@patch("traceloom.patches.patch_grpc.send_http")
@patch("traceloom.patches.patch_grpc.serialize_request_error")
def test_interceptor_captures_error_call(mock_serialize, mock_send, config):
    ...
    mock_serialize.assert_called_once()
    kw = mock_serialize.call_args[1]
    assert kw["error"] is rpc_error
    assert kw["library"] == "grpc"
    assert kw["response_status_code"] == 404
    assert kw["response_headers"] == {
        "grpc-status": "5",
        "grpc-status-name": "NOT_FOUND",
    }
    assert kw["response_body"] == "not found"
    mock_send.assert_called_once_with({"id": "err"}, context=ANY)
...
def test_error_without_code_attr_falls_back_to_unknown(
    ...
    kw = mock_serialize.call_args[1]
    assert kw["error"] is plain_error
    assert kw["response_status_code"] == 500
    assert kw["response_headers"] == {
        "grpc-status": "2",
        "grpc-status-name": "UNKNOWN",
    }
...
def test_send_capture(mock_serialize, mock_send, config):
    ...
    mock_send.assert_called_once_with({"id": "payload"}, context=context)
