"""Tests for traceloom.integrations.django — TraceLoomMiddleware."""
...
from traceloom.integrations.django import TraceLoomMiddleware  # noqa: E402
...
def captured(config):
    ...
    with (
        ...
        patch(
            "traceloom.integrations.django.send_http_incoming",
            side_effect=record,
        ),
    ):
        yield payloads
...
def _make_middleware(get_response=None, ignore_paths=None):
    ...
    if ignore_paths is not None:
        with patch.object(settings, "TRACELOOM_IGNORE_PATHS", ignore_paths, create=True):
            return TraceLoomMiddleware(get_response)
    return TraceLoomMiddleware(get_response)
...
def test_incoming_request_is_child_of_active_context(captured, factory):
...
def test_captures_exception_info(captured, factory):
    ...
    with patch(
        "traceloom.integrations.django.capture_exception",
...
def test_large_request_body_skipped(captured, factory):
    from traceloom.integrations.django import MAX_BODY_CAPTURE  # noqa: PLC0415
...
def test_process_exception_skips_ignored_paths(captured, factory):
    ...
    assert not hasattr(request, "_traceloom_exc_type")
...
def test_process_exception_respects_capture_exceptions_false(captured, config, factory):
    ...
    assert len(exception_calls) == 0
    assert request._traceloom_exc_type == "ValueError"
    assert request._traceloom_exc_value == "boom"
