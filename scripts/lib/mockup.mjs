 ...
 * @param {(page: import("puppeteer-core").Page) => Promise<void>} [opts.preparePage]
 *        Optional interaction to run after the page loads and before capture
...
export async function generateMockup({
  ...
  if (!executablePath) {
    throw new Error(
      "No Chrome installation found. Install Google Chrome or set CHROME_PATH.",
    );
  }
  ...
  try {
    const screenshotBase64 = input
      ? readFromFile(input)
      : await captureFromUrl(browser, url, {
          width,
          height,
          wait,
          preparePage,
...
async function renderMockup(
  browser,
  screenshotBase64,
  { width, height, dark, addressBar, output, transparent },
) {
