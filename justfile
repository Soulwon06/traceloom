...
# Bump traceloom client version (patch, minor, or major)
bump-client part:
    uv run --frozen bump-my-version bump {{ part }} --config-file clients/python/.bumpversion.toml

# Bump traceloom-server version (patch, minor, or major)
bump-server part:
    uv run --frozen bump-my-version bump {{ part }} --config-file server/.bumpversion.toml

# Run traceloom server locally with auto-reload (http://localhost:5110)
server:
    uv run traceloom-server --reload

# Export the OpenAPI schema (consumed by the frontend's openapi-typescript step)
openapi-export:
    uv run --frozen traceloom-server openapi-export --output frontend/openapi.json
...
frontend-bundle: frontend-build
    rm -rf server/src/traceloom_server/_frontend
    cp -r frontend/dist server/src/traceloom_server/_frontend
