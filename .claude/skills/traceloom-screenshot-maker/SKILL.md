---
name: traceloom-screenshot-maker
description: >
  Generate or regenerate screenshots for the TraceLoom landing page and docs.
  Covers both the dashboard screenshot (TraceLoom UI) and the Claude Code session
...
Generate screenshots for the TraceLoom landing page and integration guides. Two types exist.
...
## 1. Dashboard screenshot (TraceLoom UI)

Captures the live TraceLoom frontend with demo data populated.

**Prerequisites**: traceloom-server running on :5110, frontend dev server on :5111.
...
This clears all TraceLoom data, runs the demo script (`examples/python/all_in_one.py`),
...
```markdown
![TraceLoom dashboard showing captured requests](assets/debug-requests-dashboard.png)
...
- **Integration guides** (`docs/guides/debug-*.md`):
  ...
  - Claude Code screenshot after the `/traceloom` example
